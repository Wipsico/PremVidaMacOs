/**
 * Prem Vida - Ordenes, historial operativo y venta por codigo.
 *
 * NOTE: This module was migrated to use the live tables `orders` and
 * `order_items` for all active lookups, rendering, and status updates.
 * Legacy naming is no longer used in this file.
 */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const mockOrders = [
        { id: 'PV-0041', entity: 'Distribuidora Vegana Oriente', type: 'Bill (Gasto)', amount: 1420.00, status: 'Completado', raw: null },
        { id: 'PV-0042', entity: 'Cliente WhatsApp', type: 'Sale (Venta)', amount: 185.50, status: 'Despachado', raw: null },
        { id: 'PV-0043', entity: 'Mariela Justiniano', type: 'Sale (Venta)', amount: 90.00, status: 'Pendiente', raw: null },
    ];
    ...
    function renderOrders(rows) {
        ...
        tableBody.innerHTML = rows.map((order) => {
            const isPending = ['pendiente', 'espera_aprobacion', 'draft'].includes(String(order.status).toLowerCase());
            ...
            return `
                <tr class="border-b border-zinc-800/40 hover:bg-zinc-900/20 transition">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-xs px-2.5 py-1 rounded-full font-medium ${isPending ? 'bg-amber-500/10 text-amber-400' : 'bg-emerald-500/10 text-emerald-400'}">${escapeHtml(order.status)}</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <button data-order-id="${escapeHtml(order.raw?.id || order.id)}" class="btn-view-order text-zinc-400 hover:text-white transition material-symbols-outlined text-sm">visibility</button>
                    </td>
                </tr>`;
        }).join('');
    ...
    function updateKpis(rows) {
        ...
        document.getElementById('kpi-total-orders') && (document.getElementById('kpi-total-orders').textContent = rows.length);
        document.getElementById('kpi-pending-orders') && (document.getElementById('kpi-pending-orders').textContent = pending);
        document.getElementById('kpi-leader-brand') && (document.getElementById('kpi-leader-brand').textContent = labels[leader] || leader);
    }
    ...
    function openBrandPrompt() {
        ...
        button.addEventListener('click', () => window.switchTab(slug));
    ...
    async function searchOrderCode() {
        ...
        if (!code) {
            renderCodeResult('warning', 'Ingresa un codigo de venta.');
            return;
        }
        if (!supabase) {
            renderCodeResult('error', 'Supabase no esta disponible en esta sesion.');
            return;
        }
        ...
        try {
            ...
            confirmBtn.disabled = order.status === 'confirmado';
            renderOrderCodeResult(order);
        } catch (error) {
            console.error('[orders] Error buscando codigo:', error);
            renderCodeResult('error', error.message || 'No se encontro el ticket.');
        }
    }

    async function confirmCurrentOrder() {
        if (!currentCodeSale || !supabase) return;
        ...
        try {
            const { data, error } = await supabase
                ...
                .eq('id', currentCodeSale.id)
                .select()
                .single();

            if (error) throw error;
            currentCodeSale = data;
            window.showToast?.('check_circle', 'Pago registrado correctamente.', 'success');
            await searchSaleCode();
            await loadOrders();
        } catch (error) {
    ...
    async function openTransactionDetail(orderId) {
        ...
        if (!supabase || !sale) {
            ...
            openTransactionModal('Detalle de Transaccion', buildSaleDetailHtml(mock, true));
            return;
        }
        ...
        try {
            ...
            if (!detail.items?.length) {
                ...
                openTransactionModal(`Ticket ${escapeHtml(detail.order_code || sale.order_code || sale.id)}`, buildSaleDetailHtml(mock, true));
                return;
            }
    ...
    function createMockSaleDetail(orderId, sale = {}) {
        ...
        return {
            ...
            payment_method: sale.payment_method || 'Pago Movil',
            ...
            <section class="space-y-6">
                ...
                <div class="grid sm:grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="rounded-2xl bg-zinc-950/80 border border-zinc-800 p-4">
                        ...
                        <p class="text-xs text-zinc-500 mt-1">${escapeHtml(sale.payment_method || 'Pago Movil')}</p>
                    </div>
                    <div class="rounded-2xl bg-zinc-950/80 border border-zinc-800 p-4">
                        <p class="text-xs uppercase tracking-widest text-zinc-500">Método de Pago</p>
                        <p class="mt-2 text-sm font-medium text-white">${escapeHtml(sale.payment_method || 'Pago Movil')}</p>
                    </div>
                    <div class="rounded-2xl bg-zinc-950/80 border border-zinc-800 p-4">
                ...
                <div class="rounded-3xl border border-zinc-800 overflow-hidden">
                    <table class="w-full border-collapse text-left text-sm">
                        ...
                        <tbody class="bg-zinc-950/80">
                            ${itemsHtml || '<tr><td colspan="5" class="px-4 py-6 text-center text-zinc-500">No hay items registrados en esta orden.</td></tr>'}
                        </tbody>
                    </table>
                </div>
    ...
    function openTransactionModal(title, body) {
        ...
        document.getElementById('transaction-modal-body').innerHTML = body;
